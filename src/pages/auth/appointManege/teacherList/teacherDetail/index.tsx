...
export default function Index() {
  ...
  useEffect(() => {
    ...
    handleImgUrl(state.avatar, setUpFileList);
  }, []);
  // 提交
  ...
  // 切换时间段上下班状态
  const changeOnline = (outIndex: any, period: any, bottomItem: boolean) => {
    ...
    changeOnlineTime(data, dispatch).then((res) => {
      let newTimeData = onlineList;
      newTimeData[outIndex][period] = !newTimeData[outIndex][period];
  ...
  return (
    <div className="appoint-teacher">
      <div style={{ display: "flex" }}>
        <Col style={{ flex: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            {formDisable ? (
              ...
              <div>
                <Button size="small" type="default" onClick={concelModify}>
                  取消
                </Button>
                <Popconfirm
                  placement="topRight"
            ...
            >
              <ImgCrop>
                <Upload
                  ...
                  maxCount={1}
                  customRequest={(e) =>
                    uploadImageBtn(e, setUpFileList, setAvator, form)
                  }
                  onRemove={() => {
                    setUpFileList([]);
        ...
        <Col className="time-choose" style={{ paddingLeft: "2rem" }}>
          <span>导读师上班时段</span>
          ...
          <div>
            {onlineList.map((item, outIndex) => (
              <div
                style={{
                  display: outIndex == swiperCurrent ? "flex" : "none",
                  flexWrap: "wrap",
                ...
                {item.map((bottomItem, index) => {
                  return (
                    <Popconfirm
                      ...
                      title={"是否修改该时段上下班状态"}
                      onConfirm={() =>
                        changeOnline(outIndex, index, bottomItem)
                      }
                      okText="确认"
                      cancelText="取消"
                    >
                      <div
                        ...
                        key={"time" + index}
                      >
                        {10 + Math.floor(index / 2)}:{0 + (index % 2) * 3}0 -{" "}
                        {10 + Math.floor((index + 1) / 2)}:
                        {0 + ((index + 1) % 2) * 3}0
                      </div>
                    </Popconfirm>
                  );
