...
export default function Index() {
  ...
  // 绑定表单
  const [form] = Form.useForm<addContent>();
  ...
  // 提交
  const { mutateAsync: editContent } = useEditContent([
    "appointmanage",
    "contentlist",
    state.page,
    state.state,
      ...
      >
        ...
        >
          <Select style={{ width: "14rem" }}>
            <Option value="故事会">故事会</Option>
            <Option value="手偶剧">手偶剧</Option>
            <Option value="春山探索">春山探索</Option>
            <Option value="拾阅探索">拾阅探索</Option>
            <Option value="点读">点读</Option>
            <Option value="好书分享">好书分享</Option>
          </Select>
        </Form.Item>
        <Form.Item
          label="内容标题"
        ...
        >
          <ImgCrop>
            <Upload
              ...
              maxCount={1}
              customRequest={(e) =>
                uploadImageBtn(e, setUpFileList, setCover, form)
              }
              onRemove={() => {
                setUpFileList([]);
        ...
        >
          ...
          <TimePicker.RangePicker
            ...
            defaultValue={[
              moment(state.time.substring(0, 5), "HH:mm"),
              moment(state.time.substring(6), "HH:mm"),
            ]}
