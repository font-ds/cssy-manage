...
import {Table,Button,Divider, message,PaginationProps,Input,Select,InputRef,Popconfirm} from 'antd'
import {useNavigate} from 'react-router-dom';
import {useGetLeadRecordAll,useChangeLead, useConcelLead, useLongLead} from './util';
...
export default function Index() {
  ...
  // 延长借阅
  const {mutateAsync:longLead}=useLongLead(['leadmanage','leadrecordlist',page,state,keyword,type])
  ...
  <>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
      <div className='type-button'>
        <div className='title' style={{width:'5rem'}}>查找</div>
        <Input.Group className='group' compact>
          ...
          <Input ref={InputRef} onPressEnter={e=>{setKeyword(e.target.value);setPage(0);setType(currentType)}} style={{ width: '45%',height:'2rem' }}  />
          <img onClick={()=>{setKeyword(InputRef?.current?.input?.value);setPage(0);setType(currentType)}} src={require('../../../../assets/search.png')} alt='搜索'></img>
        </Input.Group>
        <Button type={state==-1?'primary':'default'} onClick={()=>{setState(-1);setPage(0)}}>所有</Button>
        <Button type={state==0?'primary':'default'} onClick={()=>{setState(0);setPage(0)}}>待取书</Button>
        <Button type={state==1?'primary':'default'} onClick={()=>{setState(1);setPage(0)}}>已借阅</Button>
        <Button type={state==2?'primary':'default'} onClick={()=>{setState(2);setPage(0)}}>已逾期</Button>
      </div>
      <Button size='small' type='primary' onClick={()=>navigate('/leadmanage/leadmanage2')}>快捷借阅书籍</Button>
    </div>
                  ...
                  <Popconfirm
                      ...
                      title={
                        <div>
                          ...
                          <div>
                            <Button type={longer=='1'?'primary':'default'} style={{margin:'1rem 0.5rem'}} onClick={()=>setLonger('1')}>1周</Button>
                            <Button type={longer=='2'?'primary':'default'} style={{margin:'1rem 0.5rem'}} onClick={()=>setLonger('2')}>2周</Button>
                            <Button type={longer=='3'?'primary':'default'} style={{margin:'1rem 0.5rem'}} onClick={()=>setLonger('3')}>3周</Button>
                            <Button type={longer=='4'?'primary':'default'} style={{margin:'1rem 0.5rem'}} onClick={()=>setLonger('4')}>4周</Button>
                          </div>
                        </div>
                      }
