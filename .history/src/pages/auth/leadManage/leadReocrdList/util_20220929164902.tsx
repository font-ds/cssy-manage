...
// 获取记录
// 分页并区分状态
export const useGetLeadRecordAll = (
  page: any,
  state: any,
  keyword: any,
  type: any
  ...
  return useQuery(
    ["leadmanage", "leadrecordlist", page, state, keyword, type],
    async () => {
      let res = await request(
        `/back/borrow?page=${page}&state=${state}&limit=20`
      );
      // 同时返回table表格pagenation设置
      let data = {
        pagination: { totle: res.page * 20, pageSize: 20, current: page + 1 },
        data: res.data,
      };
      return data;
    }
...
export const useChangeLead = (queryKey: QueryKey) => {
  ...
  return useMutation((params: bookLeadRecord) => {
    return request(
      `/back/borrow?borrow_id=${params.borrow_id}&state=${params.state}`,
      { method: "put" }
    );
...
export const useConcelLead = (queryKey: QueryKey) => {
  ...
  return useMutation((params: bookLeadRecord) => {
    let state;
    return request(
      `/back/borrow?borrow_id=${params.borrow_id}&state=${state}`,
      { method: "delete" }
    );
  }, useEditConfig(queryKey, "borrow_id"));
};
...
export const useLongLead = (queryKey: QueryKey) => {
  ...
  return useMutation((params: bookLonger) => {
    let state;
    return request(
      `/back/borrow/extend?borrow_id=${params.borrow_id}&state=${state}`,
      { method: "put" }
    );
  }, useEditConfig(queryKey, "borrow_id"));
};
