...
export default function Index() {
  ...
  <>
    <div style={{display:'flex',justifyContent:'space-between'}}>
      <div>
        <Button type={state==-1?'primary':'default'} style={{borderBottom:'0'}} onClick={()=>setState(-1)}>所有</Button>
        <Button type={state==0?'primary':'default'} onClick={()=>setState(0)}>普通用户</Button>
        <Button type={state==1?'primary':'default'} onClick={()=>setState(1)}>VIP</Button>
        <Button type={state==2?'primary':'default'} onClick={()=>setState(2)}>SVIP</Button>
      </div>
