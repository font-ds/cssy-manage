...
export default function Index() {
    ...
    // 搜索分类
    const [currnetSearchClass,setCurrnetSearchClass]=useState('0')
  // 展示类型
  ...
  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
      <div className='type-button'>
        ...
        <Input.Group className='group' compact>
          <Select defaultValue={searchClass} onChange={(e)=>setCurrnetSearchClass(e)}>
            <Option value="0">姓名</Option>
            <Option value="1">手机号</Option>
          </Select>
          <Input ref={InputRef} onPressEnter={e=>{setKeyword(e.target.value);setPage(0);setSearchClass(currnetSearchClass)}} style={{ width: '45%',height:'2rem' }}  />
          <img onClick={()=>{setKeyword(InputRef?.current?.input?.value);setPage(0);setSearchClass(currnetSearchClass)}} src={require('../../../../assets/search.png')} alt='搜索'></img>
        </Input.Group>
      </div>
      <></>
    </div>
