...
export const handleImgUrl=(key:string,func:Function,title:string='')=>{
  if(key.substring(0,4)!='http') {
    getImage(key,(url:string)=>{
      if(title=='') func([{state:'done',url}])
      else func([{state:'done',url,name:title,type:'vedio/mp4'}])
    })
  }
  else{
    if(title=='') func([{state:'done',url:key}])
    else func([{state:'done',name:title,url:key,type:'vedio/mp4'}])
  } 
}