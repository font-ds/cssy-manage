...
import {Button,Divider,Form,Input,FormInstance,Radio,Upload, Col,Tag, message} from 'antd';
...
export default function Index() {
    ...
    // 提交
    const {mutateAsync:editBook}=useEditBook(['bookmanage','booklist'])
    const handleSubmit= ()=>{
        ...
        <Form
            ...
            >
                ...
                <Form.Item
                  label="书籍分类"
                >
                   <Col span={24} style={{display:'flex',alignItems:'center',flexWrap:'wrap'}}>
                    {
                      tags.map((item,index)=><Tag style={{margin:'0.5rem 0.2rem'}} closable onClose={()=>{let newTags=tags.splice(0,index);setTags(newTags)}} key={index}>{item}</Tag>) 
                    }
                    {
                      addTag?<Input
                      type="text"
                    ...
                    />:<Tag style={{marginLeft:'0.1rem'}} onClick={()=>setAddTag(true)}><PlusOutlined /></Tag>
                    }
                  </Col>
                </Form.Item>
                <Form.Item
