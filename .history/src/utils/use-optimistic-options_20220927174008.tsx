...
export const useEditConfig = (queryKey: QueryKey, key: string = "id") =>
  useConfig(
    queryKey,
    (target, old) => {
      if(!old?.length) return  old?.data?.map((item: any) =>
      item[key] === target[key] ? { ...item, ...target } : item
     ) || []
      else return  old?.map((item: any) =>
      item[key] === target[key] ? { ...item, ...target } : item
     ) || []
    }
...
export const useDeleteConfig = (queryKey: QueryKey, key: string = "id") =>
  useConfig(queryKey, (target, old) => {
    ...
    if (target instanceof Array) {
      ...
      if(!old?.length)
        return old?.data?.filter((item: any) => item[key] !== target[key]) || [];
...
export const useAddConfig = (queryKey: QueryKey) =>
  useConfig(queryKey, (target, old) => {
    if(old.length) return (old ? [target,...old] : [])
    else return (old?.data ? [target,...old?.data] : [])
  }
);